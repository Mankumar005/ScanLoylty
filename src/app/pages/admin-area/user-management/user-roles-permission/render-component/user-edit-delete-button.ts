...
export class UserRoleRenderComponent {
...
public onEditUser(event:any):void {
    this.router.navigate([`/pages/admin-area/user-management/add-edit-user-role`], { queryParams: {roleId : btoa(event.id)}});
}
  ...
  this.dialogService.open(ConfirmModalComponent, {
    ...
    .onClose.subscribe((confirm: any) => {
      if (confirm) {
       ...
       this.userRoleService.onDeleteUserRole(event.id).subscribe(
              (res: any) => {
