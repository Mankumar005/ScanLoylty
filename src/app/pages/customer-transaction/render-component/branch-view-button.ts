...
@Component({
  ...
  template: `
  <nb-icon class="nb-view pointer view-icon" nbTooltip="View Cycle" nbTooltipPlacement="bottom" icon="eye-outline" (click)="onViewCustomerDetails(rowData)"
   ></nb-icon>
  `,
})
export class BranchRenderComponent {
  ...
  public onViewCustomerDetails(event:any):void {
    let branchObj :any = {};
    branchObj.view = 'view-cycle',
    branchObj.id = event.id
    localStorage.setItem('branchObj',JSON.stringify(branchObj));
    this.utilService.storeEventData.next('view-cycle');
   }
