...
@Component({
  ...
  template: `
  <nb-icon class="nb-view pointer view-icon" nbTooltip="View Branch" nbTooltipPlacement="bottom" icon="eye-outline" (click)="onViewCustomerDetails(rowData)"
   ></nb-icon>
  `,
})
...
public onViewCustomerDetails(event:any):void {
  let companyObj :any = {};
  companyObj.view = 'view-branch',
  companyObj.id = event.id
  localStorage.setItem('companyObj',JSON.stringify(companyObj));
 this.utilService.storeEventData.next('view-branch');
}
